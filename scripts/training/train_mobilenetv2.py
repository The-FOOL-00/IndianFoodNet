# Baseline: MobileNetV2 with frozen pretrained backbone, only final classifier trained.
...
MODEL_DIR   = "/content/models/mobilenetv2"
...
# Replace final classifier — only this layer will be trained
# MobileNetV2 classifier is Sequential([Dropout, Linear]); index 1 is the Linear
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    if val_acc > best_val_acc:
        ...
        shutil.copy(BEST_MODEL,
                    f"{DRIVE_DIR}/models/best_model_mobilenetv2.pth")
...
results = {
    "model":                  "MobileNetV2 Baseline (frozen backbone)",
...
metrics_path = os.path.join(METRICS_DIR, "mobilenetv2_results.json")
...
plt.tight_layout()
acc_plot = os.path.join(PLOTS_DIR, "mobilenetv2_accuracy.png")
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
loss_plot = os.path.join(PLOTS_DIR, "mobilenetv2_loss.png")
...
plt.ylabel("True")
plt.tight_layout()
cm_plot = os.path.join(PLOTS_DIR, "mobilenetv2_confusion.png")
...
print(sep)
print(f"  Model          : MobileNetV2 (frozen backbone)")
...
shutil.copy(metrics_path,
            f"{DRIVE_DIR}/results/mobilenetv2_results.json")
for plot in ["mobilenetv2_accuracy.png",
