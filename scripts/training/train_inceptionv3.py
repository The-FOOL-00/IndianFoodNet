# Baseline: InceptionV3 with frozen pretrained backbone, only final classifiers trained.
# InceptionV3 requires 299x299 input and has an auxiliary head used during training.
# Compared against ResNet-50 baseline and proposed EfficientNet-B0.
...
MODEL_DIR   = "/content/models/inceptionv3"
...
# ─── Data transforms ──────────────────────────────────────────────────────────
# InceptionV3 was trained on 299x299 images; use that size for correct feature alignment.
...
# Replace both classifier heads — both will be trained.
# The auxiliary head (AuxLogits.fc) is only active during model.train().
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    for images, labels in train_loader:
        ...
        # During training InceptionV3 returns InceptionOutputs(logits, aux_logits).
        # Combine main and auxiliary losses (standard weighting from the original paper).
    ...
    if val_acc > best_val_acc:
        ...
        shutil.copy(BEST_MODEL,
                    f"{DRIVE_DIR}/models/best_model_inceptionv3.pth")
...
results = {
    "model":                  "InceptionV3 Baseline (frozen backbone)",
...
metrics_path = os.path.join(METRICS_DIR, "inceptionv3_results.json")
...
plt.tight_layout()
acc_plot = os.path.join(PLOTS_DIR, "inceptionv3_accuracy.png")
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
loss_plot = os.path.join(PLOTS_DIR, "inceptionv3_loss.png")
...
plt.ylabel("True")
plt.tight_layout()
cm_plot = os.path.join(PLOTS_DIR, "inceptionv3_confusion.png")
...
sep = "=" * 55
print(sep)
print("  INCEPTIONV3 — FINAL RESULTS")
print(sep)
print(f"  Model          : InceptionV3 (frozen backbone)")
...
shutil.copy(metrics_path,
            f"{DRIVE_DIR}/results/inceptionv3_results.json")
for plot in ["inceptionv3_accuracy.png",
             "inceptionv3_loss.png",
             "inceptionv3_confusion.png"]:
